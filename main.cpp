...
using namespace std;
    ...
    //a buffer with all the possible adresses
    ...
    //send the data to the flashchip
    for (int i = 0; i < 32; i++){
        int rand1 = rand() % 9;
        int rand2 = rand() % 9;
        int rand3 = rand() % 16;
        ...
        char rand_3 = (rand3 & char_max);
        char rand_4 = (rand4 & char_max);
        char send_buffer[6] = { 0x00, adress_buffer[i] , rand_1, rand_2, rand_3, rand_4 };
        write(i2cfile, send_buffer, 6);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    //print the values from the flash chip via serial communication
    char print_buffer[] = { 0x03 };
    write(i2cfile, print_buffer, 1)

    std::this_thread::sleep_for(std::chrono::milliseconds(1000));

    //get the values out of the flash chip
    for (int i = 0; i < 32; i++){
    ...
    //clear the flash chip (everything to 0)
    char clear_buffer[] = { 0x02 };
    write(i2cfile, clear_buffer, 1)
